...
// here we are learning how string do magic and saving resources by returning by reference and not by value as in case of integers 
...
template <typename T>
const T& address(const T& a,const T& b)
{
    cout<<&a<<endl;
}

template <typename T>
T& addressnoconst(T& a,T& b)
{
    ++a;
    cout<<&a<<endl;
}
...
*/
    ...
    //incase of string, if we return a value, then it is pass by reference and not pass by value 
    //lets see this 
    //incase of integers
    ...
    int aaa=5;
    int bbb=10;
    int result=addr(aaa,bbb);
    cout<<&result<<endl;

    
    string ccc="hello";
    string ddd="world";
    string result1=addr(ccc,ddd);
    cout<<&result1<<endl;
    ...
    auto cool50=[](int a,int b){return a+b;};
    cout<<cool50(2,3)<<endl;

    int nice=5;
    auto cool51=[nice](int a,int b)->int{return a+b+nice;}(2,3);
    // the above was pass by value to capture variable 
    // now lets see pass by reference
    cout<<cool51<<endl;
    auto cool52=[&nice](int a,int b)->int{return a+b+nice;}(2,3);
    // the above was pass by reference to capture variable
    cout<<cool52<<endl;

    // now lets see hoow to capture all values

    auto cool53=[&](int a,int b)->int{return a+b+nice;}(2,3);
    // the above was pass by reference to capture all variables

    // now lets see hoow to capture all values

    auto cool54=[=](int a,int b)->int{return a+b+nice;}(2,3);


    // using function templates 
    // these are very handy as they us from repetetive code and also save us from writing multiple functions for different data types
    // like suppose we want to add two numbers, then we can write a function for int, float, double etc etc

    cout<<"using function templates"<<endl;

    cout<<add(2,3)<<endl;
    cout<<add(2.5,3.5)<<endl;
    cout<<add(2.5f,3.5f)<<endl;
    cout<<add(2.5L,3.5L)<<endl;
    ...
    maximum(p,q); // int type deduced

    maximum(r,s); // double type deduced

    maximum(ee,ff); // string type deduced

    maximum<double>(r,s); // explicitly say that we want the double // version called, if an instance is not there // already, it will be created. maximum<double>(a,c); // Works, even if parameters are of different types, // there is implicit conversion from int to double // for the first parameter.

    //maximum<double>(a,ee); // Error : can’t convert std::string to double.

    // using const referencing in templare and non const referencing in template

    cout<<"using const referencing in templare"<<endl;
    ...
    cout<<address(aaaa,bbbb)<<endl; // int type deduced
    cout<<address<double>(aaaa,bbbb)<<endl; // double type deduced

    cout<<"using non const referencing in templare"<<endl;
    ...
    cout<<addressnoconst(aaaaa,bbbbb)<<endl; // int type deduced
